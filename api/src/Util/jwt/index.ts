...
export async function MakeTokens(user: any, userType: UserType) {
    ...
    try {
        ...
        await redisCache.removeRefreshToken(user._id);

        const ttl: number = (Jwt.decode(refreshToken) as Jwt.JwtPayload).exp ?? 145152000;

        await redisCache.addRefreshToken(refreshToken, user._id, ttl);
...
export async function verifyAccessToken<T>(token: string, userType: UserType): Promise<T> {
    ...
    try {
        ...
        if (!Object.keys((decoded?.payload as Jwt.JwtPayload)).includes("_id")) throw Error("No Valid Token");

        const userId = (decoded?.payload as Jwt.JwtPayload)._id;
